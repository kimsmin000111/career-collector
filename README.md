...
- 실행 가능한 adapter: 현대모비스·Career Doosan 공식 HTML, 재정경제부 ALIO 공식 공개 JSON, LG Careers·한화인·Samsung Careers 공식 공개 데이터, 설정형 HTML, 설정형 공공 JSON API.
- registry 항목 중 현대모비스·ALIO·LG Careers·한화인·Samsung Careers·Career Doosan 여섯 통합 출처를 활성화했습니다. 나머지는 수집 완료 기업이 아니라 정책·구조·규모·공식 주소 검토 후보입니다. 통합 출처는 여러 기관·계열사를 한 번에 확인합니다.
- LG Careers는 목록 제목만 저장하지 않고 신입·신입/경력·인턴 공고의 상세 직무를 분리합니다. 따라서 LG디스플레이의 기구·구조해석, LG전자, LG마그나, 로보스타 등의 기계 관련 세부 직무를 판정할 수 있습니다.
- 한화인은 신입·인턴 공고를 직무 단위로 분리합니다. Samsung Careers도 신입·인턴 목록과 상세 직무를 분리하며, 현재 공고가 0건인 상태도 정상으로 처리해 다음 모집 시작을 감지합니다. 두 사이트의 robots.txt 경로는 정책 문서가 아닌 SPA HTML 또는 오류 JSON을 반환하므로, 공개 채용 데이터 주소만 사용하는 출처별 기록된 예외를 적용합니다. 유효한 `Disallow` 규칙이 생기면 예외보다 우선합니다. 포스코 공식 채용 사이트는 robots.txt에서 전체 자동 접근을 금지하므로 수집하지 않습니다. SK·HD현대 등은 확인된 공식 API나 허용된 HTML 구조를 추가해야 합니다.
- Career Doosan은 공개 공식 목록에서 신입·인턴 공고를 발견하고 공고번호가 포함된 상세 페이지를 확인합니다. 경력 전용 공고와 신입 근거가 없는 전문·계약 공고는 활성 후보에서 제외합니다.
- 공공기관은 재정경제부 ALIO 공개 채용정보 JSON에서 현재 접수 중인 관련 NCS 직군(운전·운송, 건설, 기계, 재료, 화학, 전기·전자, 환경·에너지·안전, 연구)을 조회합니다. 목록 응답 자체에 기관·학력·근무지·채용구분·자격요건·우대조건·마감일이 포함되어 상세 HTML 수백 건을 반복 조회하지 않습니다. NCS가 겹치는 공고는 공고번호로 합칩니다.
- ALIO 공고는 여러 직렬·학력·전형이 섞여 있어 기본적으로 `확인 필요`로 보관합니다. 공식 공개 데이터가 일시 실패하면 기존 데이터를 보존하고 상태를 오류로 표시합니다. 다른 활성 출처가 정상 갱신되면 그 데이터는 계속 게시합니다. 첨부 PDF/HWP/ZIP을 해석해 직렬별 필수 자격을 분리하는 기능은 아직 없습니다. 따라서 신입 제목만으로 지원 가능을 보증하지 않습니다.
...
python app.py --source hanwha
python app.py --source samsung
...
`output/coverage.json`에는 전체 등록 출처의 자동 수집 여부, 최근 성공, 최근 건수, 연속 실패 횟수와 오류 유형을 기록합니다. `health.json`에는 실행 시작·종료, 소요시간, 발견·신규·수정·마감 건수와 출처별 이전/현재 건수를 기록합니다. 이전에 여러 건이던 출처가 갑자기 0건이면 정상 공고 종료로 단정하지 않고 `SOURCE_EMPTY_ANOMALY`로 남깁니다.
...
각 호스트의 robots.txt를 확인하고, 실패 시 닫힌 상태로 중단합니다(404/410은 별도 robots 규칙 없음). 공개 채용 데이터가 확인됐지만 robots 경로가 SPA 또는 오류 JSON으로 잘못 연결된 경우에만 출처 설정에 검토 기록과 명시적 예외를 둡니다. 유효한 robots 규칙은 항상 적용합니다. 호스트당 기본 2초 간격, 30초 읽기 제한, 응답 5MB 제한, 등록된 HTTPS 호스트만 허용합니다. robots crawl-delay가 더 길면 따릅니다. CAPTCHA, 로그인, 차단 우회, 프록시 회전, 이메일 수집은 구현하지 않습니다. 사람인·잡코리아도 승인된 API 없이는 추가하지 않습니다.

공공데이터라도 제3자 저작권이 포함된 첨부물은 별도 정책을 확인해야 합니다. 첨부 파일·사진·이력서는 다운로드하거나 재배포하지 않습니다. 공고의 업무·요건은 원문 링크와 함께 표시하며 원문이 최종 기준입니다.

SQLite는 `collector-data` 브랜치에 보관하므로 Actions artifact 보존기간이 끝나도 채용 이력은 유지됩니다. HTTP 캐시만 Actions cache에 저장하며 유실 시 다시 다운로드합니다. 일시적 네트워크 오류가 나면 최근 정상 응답을 사용할 수 있지만 해당 출처를 정상으로 위장하지 않고 `STALE_CACHE_FALLBACK` 부분수집으로 기록합니다. 마감일 변경은 같은 공식 URL의 기록을 갱신하고, 마감 기록은 active=false로 유지합니다. 공식 URL이 서로 다른 신입/경력 공고는 같은 직무·기간이어도 합치지 않습니다. 목록에서 사라졌다는 이유로 마감 처리하지 않습니다. 공식 조기 마감 배지가 별도 추출되지 않는 adapter는 마감일 기준으로만 종료를 판정하므로 원문 확인이 필요합니다.
